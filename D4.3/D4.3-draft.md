...
The performance bottleneck here is almost entirely dependent on the RDF library and parser used (rdflib^[https://github.com/RDFLib]).  If a different RDF library (and possibly a different programming language) were used, the performance characteristucs seen are likely to be very different.
...
\TODO{Link to component web page for developers}
...
* Web site states licence.
    * \TODO{Figure out when web site link is provided}
* Software (source and binaries) has a licence.
    * No
* Software has an open source licence.
    * Yes (MIT)
